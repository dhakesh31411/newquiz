...
const apiJsPath = path.join(__dirname, '../backend/routes/api.js');
...
let activeWebsiteName = 'QuizMaster';
  ...
  try {
    ...
    await connection.query(\`
      CREATE TABLE IF NOT EXISTS quizzes (
        ...
        description TEXT,
        image VARCHAR(255) DEFAULT '/logo.jpg',
    ...
    try { await connection.query("ALTER TABLE questions ADD COLUMN option_a_image TEXT;"); } catch (err) {}
    try { await connection.query("ALTER TABLE questions ADD COLUMN option_b_image TEXT;"); } catch (err) {}
    try { await connection.query("ALTER TABLE questions ADD COLUMN option_c_image TEXT;"); } catch (err) {}
    try { await connection.query("ALTER TABLE questions ADD COLUMN option_d_image TEXT;"); } catch (err) {}
    ...
    // Fetch existing settings if present
...
router.post('/users/register', async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ success: false, message: 'User login failed. Database error: ' + err.message });
  }
});

// Get Quizzes for User Dashboard
router.get('/quizzes', async (req, res) => {
  try {
    const [quizzes] = await pool.query('SELECT * FROM quizzes WHERE is_active = 1 ORDER BY is_quiz_of_day DESC, id DESC');
    ...
    res.status(500).json({ success: false, message: 'Failed to fetch quizzes. Database error: ' + err.message });
  }
});
...
fs.writeFileSync(apiJsPath, newCode, 'utf8');
console.log('Successfully updated backend/routes/api.js');
